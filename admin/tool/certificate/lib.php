...
function tool_certificate_leftmenunode(){
    ...
    if(has_capability('tool/certificate:manage', $systemcontext) || is_siteadmin() ) {
        $certificatenode .= html_writer::start_tag('li', array('class' => 'pull-left user_nav_div browsecertifications'));
            ...
            $certification = html_writer::link($certification_url, '<span class="certificates_icon left_menu_icons"></span><span class="user_navigation_link_text">'.get_string('manage_certificates','tool_certificate').'</span>',array('class'=>'user_navigation_link'));
