...
/**
     ...
     */
    public static function get_myselectedcourses_parameters() {
     ...
     */
    public static function get_myselectedcourses($options,$dataoptions,$offset = 0,$limit = 0, $filterdata) {
        ...
        $params = self::validate_parameters(
            self::get_myselectedcourses_parameters(),
            [
                'options' => $options,
        ...
        $stable->start = $offset;
        $stable->length = $limit;
      /*  // $selectedcourses = $DB->get_records_sql($fromsql . $sql.$ordersql, $params);
        $scourse=implode(',',$data_object->course) ;
 */

         $fromsql="SELECT * ";

       
        $totalcourselister=plugin::get_selectedcourses($stable,$filtervalues,$data_object);
        $totalcount=$totalcourselister['allcoursecount'];
       ...
       $data = array();
        ...
        for ($i=0; $i < $arraylen; $i++) {
            ($data[$i]['userid'] = $USER->id);//array_push
        }
        $nomycourselister=new \stdClass();
       // $nomycourselister->findlearninglink=html_writer::link(new moodle_url('/local/catalog/allcourses.php'),get_string('leftmenu_catalog',plugin::COMPONENT));
     //   $nomycourselister->learningtype=get_string('elearning',plugin::COMPONENT);
        return [
     ...
     */
    public static function  get_myselectedcourses_returns() {
            ...
            'records' => new external_multiple_structure(
                            new external_single_structure(
                                array(
                                    ...
                                    'course_provider' => new external_value(PARAM_RAW, 'course_provider', VALUE_OPTIONAL),
                                    'duration' => new external_value(PARAM_RAW, 'duration', VALUE_OPTIONAL),
                                    'modstr' => new external_value(PARAM_RAW, 'modstr', VALUE_OPTIONAL),
                                    ...
                                    'enrolledcount' => new external_value(PARAM_RAW, 'enrolledcount',VALUE_OPTIONAL),
                                    'completedcount' => new external_value(PARAM_RAW,'completedcount',VALUE_OPTIONAL),
