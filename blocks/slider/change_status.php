...
$baseurl        = new moodle_url('/blocks/slider/change_status.php', array('view' => 'manage', 'sliderid' => $sliderid, 'id' => $id));
$confirmurl     = new moodle_url('/blocks/slider/change_status.php',
        array('view' => 'manage', 'sliderid' => $sliderid, 'id' => $id, 'confirm' => 1));

$PAGE->navbar->add(get_string('manage_slides','block_slider'), $redirecturl);
$PAGE->navbar->add(get_string('change_status', 'block_slider'));
...
if (!$slide = $DB->get_record('slider_slides', array('id' => $id))) {
    ...
    if (!$confirm) {
        echo $OUTPUT->header();
        $slide = $DB->get_record('slider_slides', array('id' => $id));

        if ($slide->status == 0) {
            $confirm = html_writer::tag('h3', get_string('confirm_change_status0', 'block_slider'));
        }

        if ($slide->status == 1) {
            $confirm = html_writer::tag('h3', get_string('confirm_change_status1', 'block_slider'));
        }
        ...
        if ($slide->status == 1) {
            ...
            $success = $DB->update_record($dtable, $data);

            if ($success == true) {
                redirect($redirecturl, get_string('change_status_noti0', 'block_slider'), null, \core\output\notification::NOTIFY_SUCCESS);
            }
        }
        ...
        if ($slide->status == 0) {
            ...
            if ($success == true) {
                redirect($redirecturl, get_string('change_status_noti1', 'block_slider'), null, \core\output\notification::NOTIFY_SUCCESS);
            }
        }
