...
class block_slider extends block_base {
    ...
    public function get_content() {
        ...
        // Prepare slides.
        if ($slides = $DB->get_records('slider_slides', array('sliderid' => $this->instance->id, 'status' => 0), 'slide_order ASC')) {
        ...
        // $this->content->text .= $this->display_videos($bxslider);
    ...
    public function display_images($bxslider = false) {
                $this->page->requires->js_call_amd('block_slider/video_popup', 'load', array());
        global $CFG;
        // Get and display images.
        $html = '';
        if ($this->hasslides) {
            foreach ($this->hasslides as $slide) {
                // $imageurl = $CFG->wwwroot . '/pluginfile.php/' . $this->context->id . '/block_slider/slider_slides/' . $slide->id .'/' . $slide->slide_image;
                // $videourl = $CFG->wwwroot . '/pluginfile.php/' . $this->context->id . '/block_slider/slider_slides/' . $slide->id .'/' . $slide->slide_video;

                 $imageurl = slider_img_path($slide->slide_image);
                $videourl = slider_video_path($slide->slide_video);

                if (empty($slide->slide_video) && $bxslider) {
                    $html .= html_writer::start_tag('div', ['class' => 'bxslide', 'style' => "background-image: url('".$imageurl."');", 'onclick' => 'window.open("'.$slide->slide_link.'", "_blank")']);
                }
                if (!empty($slide->slide_video) && $bxslider) {
                    // $html .= $videourl;
                    $html .= html_writer::start_tag('div', ['class' => 'bxslide', 'style'=>"background-image: url('".$imageurl."');"]);
                        $html .= html_writer::start_tag('div', array('class' => 'play_class'));
                            $demo = html_writer::tag('i', '', array('class' => 'fa fa-play-circle', 'aria-hidden' => 'false'));
                            $html .= html_writer::tag('a', $demo, [
                                'title'         => get_string('play_video', 'block_slider'),
                                'class'         => 'btn_class',
                                'id'            => 'btn_id',
                                /*'href'        => 'javascript:void(0)',
                                'data-action'   => 'video_popup',*/
                                'onclick'       => "(function(e) {
                                    require('block_slider/video_popup').init( {
                                        videourl:'$videourl', contextid:1, pluginname:'block_slider'
                                    })
                                })(event)"
                            ]);
