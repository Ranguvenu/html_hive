...
class report_detailed_course_completion extends report_base{
	...
	function get_rows($elements, $sqlorder = ''){
		...
		if(!empty($elements)){			
			...
			if( !empty($category) || !empty($courseid) || !empty($completionstatus) || !empty($emp_status) || (!empty($filter_starttime) && !empty($filter_endtime)) || !empty($userid)){
				...
				$sql = "SELECT ue.id,ra.id, u.id as userid, 
								u.open_employeeid AS employeeno,
								concat(u.firstname ,' ',u.lastname) AS employeename,
								u.email, u.open_costcenterid,u.open_departmentid,
								c.category,u.open_designation, u.open_grade,u.open_careertrack,u.open_location,
								c.id AS courseid,c.idnumber as courseidnumber,
								c.fullname AS course,
								c.shortname,u.suspended,c.open_identifiedas, c.open_points,c.open_courseprovider,c.open_ouname,c.open_url,c.autoenrol,c.selfenrol,
								c.duration,c.visible,
								cc.timecompleted AS completionstatus, 
								cc.timecompleted AS completiondate,
								ue.timecreated AS enrolleddate,
								c.open_skill,c.open_skillcategory,c.open_level,
                        ...
                        JOIN {user} AS u ON u.id = ra.userid and u.id = ue.userid 
				...
				if($courseusers){
					foreach($courseusers as $enroluser){
						...
						if(empty($enroluser->open_location)){
							$manger->employeelocation = 'NA';
						}else{
							$manger->employeelocation = $enroluser->open_location;
						}
						if(empty($enroluser->courseidnumber)){
							$manger->courseidnumber = 'NA';
						}else{
							$manger->courseidnumber = $enroluser->courseidnumber;
						}
						...
						if($enroluser->open_ouname == '-1'){
							$manger->ouname = 'ALL';
						}else if($enroluser->open_ouname != 0 || $enroluser->open_ouname != NULL){
						...
						$manger->employeeid = trim($enroluser->employeeno);
					...
					/* 	$manger->approvername =  $DB->get_field_sql("SELECT concat(firstname ,' ',lastname) 
													FROM {user} u 
													JOIN {grade_grades} gg ON u.id = gg.usermodified 
													JOIN {grade_items} gt ON gt.id = gg.itemid AND gt.courseid = $enroluser->courseid
													WHERE gg.userid = $enroluser->userid AND gt.itemmodule = 'assign' "); */
						$manger->approvername = $DB->get_field_sql("SELECT concat(firstname ,' ',lastname) 
										FROM {user} u 
										JOIN {assign_grades} ag ON ag.grader = u.id 
										JOIN {course_modules} cm ON cm.instance = ag.assignment 
										WHERE course = :courseid AND cm.module = :moduleid AND ag.userid = :userid",array('courseid' => $enroluser->courseid,'moduleid' => 1,'userid' => $enroluser->userid));
		
						$manger->approvername = ($manger->approvername) ? $manger->approvername : 'N/A';
						$reportarray[] = $manger;
					}
				}
			}
