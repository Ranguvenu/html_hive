...
class plugin_categories extends plugin_base {
    ...
    public function init() {
        ...
        $this->reporttypes = array('categories', 'sql','detailed_course_completion','course_certification');
    }
    ...
    public function print_filter(&$mform) {
        ...
        $categorieslist = $remotedb->get_records_menu('course_categories',array(),'name','id,name');

        $courseoptions = array();
        $courseoptions[null] = get_string('select_option', 'block_configurable_reports');
        $courseoptions[-1] = get_string('filter_all', 'block_configurable_reports');

        $categoriesoptions = $courseoptions + $categorieslist;

        $mform->addElement('select', 'filter_categories', get_string('category'), $categoriesoptions);
        $mform->setType('filter_categories', PARAM_INT);

        $mform->addElement('html', html_writer::script(" $('#id_filter_categories').select2();"), array());
    }
}
