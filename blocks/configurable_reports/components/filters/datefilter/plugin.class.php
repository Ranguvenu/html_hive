...
class plugin_datefilter extends plugin_base {

    public function init() {
        $this->form = false;
        $this->unique = true;
        $this->fullname = get_string('startendtime', 'block_configurable_reports');
        $this->reporttypes = array('idp');
    }
    ...
    function print_filter(&$mform) {
        ...
        if($report_type =='idp'){
                $mform->addElement('html',html_writer::tag('label','<b>ILP created duration</b>',array('id'=>'filter_label')));
        }

        $mform->addElement('date_selector', 'filter_starttime', 'From',array('startyear' => 2016, 
                                                                                'stopyear'  => date('Y',time()),
                                                                                'optional'  => TRUE));
        $mform->disabledIf('filter_starttime', 'filter_starttime', 'eq', COMPLETION_TRACKING_NONE);
        
        $mform->addElement('date_selector', 'filter_endtime', 'To',array('startyear' => 2016, 
                                                                            'stopyear'  => date('Y',time()),
                                                                            'optional'  => TRUE));
