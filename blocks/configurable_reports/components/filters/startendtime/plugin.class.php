...
class plugin_startendtime extends plugin_base {
    ...
    public function init() {
        ...
        $this->reporttypes = array('sql', 'timeline', 'users', 'courses', 'detailed_course_completion','course_certification');
    }
    ...
    function print_filter(&$mform) {
        ...
        if($this->report->type == 'detailed_course_completion'){
            $mform->addElement('html',html_writer::tag('label','<b>Enrolment Period</b>',array('id'=>'filter_label')));
        }
        $from = get_string('starttime', 'block_configurable_reports');
        $to = get_string('endtime', 'block_configurable_reports');

        $mform->addElement('date_time_selector', 'filter_starttime',$from,array('optional'=>true));
        $mform->disabledIf('filter_starttime', 'filter_starttime', 'eq', COMPLETION_TRACKING_NONE);
        //$mform->setDefault('filter_starttime', time() - 3600 * 24);
        $mform->addElement('date_time_selector', 'filter_endtime', $to,array('optional'=>true));
