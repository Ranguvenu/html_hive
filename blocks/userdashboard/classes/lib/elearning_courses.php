...
class elearning_courses{
    ...
    public static function courses_todo() {
        ...
        $sql = "SELECT count(distinct(course.id)) as countid
                ...
                AND ( CONCAT(',',course.open_identifiedas,',') LIKE '%,3,%' OR CONCAT(',',course.open_identifiedas,',') LIKE '%,1,%' )
    ...
    public static function tocheckcompleted($userid) {
        ...
        $sql1 = "SELECT distinct(course.id),course.id as name
                ...
                AND ( CONCAT(',',course.open_identifiedas,',') LIKE CONCAT('%,',3,',%') OR CONCAT(',',course.open_identifiedas,',') LIKE CONCAT('%,',1,',%') ) AND course.visible = 1 ";
    ...
    public static function inprogress_coursenames($filter_text='', $mobile = false, $status = 'inprogress', $limit = false, $type = '', $page = 0, $perpage = 10) {
        global $DB, $USER;
            ...
            $sql .= " WHERE ue.userid = $USER->id AND course.open_costcenterid = $USER->open_costcenterid AND (CONCAT(',',course.open_identifiedas,',') LIKE CONCAT('%,',3,',%') OR CONCAT(',',course.open_identifiedas,',') LIKE CONCAT('%,',1,',%')) AND course.id>1";
    ...
    public static function inprogress_coursenames_count($filter_text){
        ...
        $sql = "SELECT COUNT(course.id)  FROM {course} AS course
            ...
            AND (CONCAT(',',course.open_identifiedas,',') LIKE CONCAT('%,',3,',%') OR CONCAT(',',course.open_identifiedas,',') LIKE CONCAT('%,',1,',%') )
