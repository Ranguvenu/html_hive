...
function check_event_access($event) {
   ...
   //                  	$training_info->trianers = 'N/A';
		...
		if (!$enrolled) { // check if the plugin allows self enrollment			
			if ($event->plugin ==='local_classroom' ) {
...
function common_access_sql() {
    ...
    if($plugins['onlinetest']){
		$sql_array[] = " WHEN e.plugin = 'local_onlinetests' 
							THEN (SELECT o.id 
								FROM {local_onlinetest_users} o 
								WHERE o.onlinetestid = e.plugin_instance 
								AND userid = {$USER->id}) ";
	}
	if($plugins['feedback']){
		$sql_array[] = " WHEN e.plugin = 'local_evaluation' 
						THEN (SELECT o.id 
								FROM {local_evaluation_users} o 
								WHERE o.evaluationid = e.plugin_instance AND userid = {$USER->id}) ";
	}
	if($plugins['classroom']){
		$sql_array[] = " WHEN e.plugin = 'local_classroom' 
						THEN (SELECT lcu.id 
								FROM {local_classroom_users} lcu 
								JOIN {local_classroom} lc ON lc.id = lcu.classroomid 
								WHERE lcu.classroomid = e.plugin_instance AND lc.status = 1 AND lcu.userid = {$USER->id} ) ";
	}

	$sql_array[] = " WHEN e.modulename IS NOT NULL 
					THEN (SELECT ue.id 
							FROM {course} as c 
							...
							AND ( CONCAT(',',c.open_identifiedas,',') LIKE '%,3,%' OR CONCAT(',',c.open_identifiedas,',') LIKE '%,1,%') ) ";
