...
function xmldb_format_toggletop_upgrade($oldversion = 0) {

    global $DB;
    $dbman = $DB->get_manager();
    $result = true;

    /* From Moodle 2.2 bit, this places the right defaults in the 'format_toggletop_settings' table so they can be read by the 2.3
       update code even though the table is then dropped.... */
    if ($result && $oldversion < 2012070300) {
        // Rename table format_toggletop_layout if it exists.
        $table = new xmldb_table('format_toggletop_layout');
        // Rename the table...
        if ($dbman->table_exists($table)) {
            $dbman->rename_table($table, 'format_toggletop_settings');
        }
        $table = new xmldb_table('format_toggletop_settings');   // Use the new table.
        ...
        // Changing sign of field id on table format_toggletop_settings to signed - mysql only,
        ...
        if ($DB->get_dbfamily() == 'mysql') {
            ...
            // Changing sign of field courseid on table format_toggletop_settings to signed.
            ...
            // Changing sign of field layoutelement on table format_toggletop_settings to signed.
            ...
            // Changing sign of field layoutstructure on table format_toggletop_settings to signed.
        ...
        // Define field tgfgcolour to be added to format_toggletop_settings.
        ...
        // Define field tgbgcolour to be added to format_toggletop_settings.
        ...
        // Define field tgbghvrcolour to be added to format_toggletop_settings.
        ...
        // New field layoutcolumns on table format_toggletop_settings.  This is not the same place as install.xml
        ...
        // Drop table format_toggletop_cookie_cnsnt if it exists - this may not work, please check db to see that
        // the table has really gone.
        $table = new xmldb_table('format_toggletop_cookie_cnsnt');
    ...
    if ($result && $oldversion < 2012120100) { // Note to self, Moodle 2.3 version cannot now be greater than this.
        $table = new xmldb_table('format_toggletop_settings');
        if ($dbman->table_exists($table) == true) {
            ...
            foreach ($records as $record) {
                ...
                if ($DB->record_exists('course', array('id' => $record->courseid))) {
                    ...
                    // Only update if the current format is 'toggletop' as we must have an instance of 'format_toggletop' (in 'lib.php')
                    // returned by the above.  Thanks to Marina Glancy for this :).
                    // If there are entries that existed for courses that were originally toggletop, then they will be lost.  However
                    ...
                    // to another format.
                    if ($courseformat->get_format() == 'toggletop') {
                        $courseformat->restore_toggletop_setting($record->courseid, $record->layoutelement, $record->layoutstructure,
                                                               $record->layoutcolumns, $record->tgfgcolour, $record->tgbgcolour,
                                                               $record->tgbghvrcolour); // In '/course/format/toggletop/lib.php'.
                    }
                }
            }
    ...
    if ($oldversion < 2017110301) {
        ...
        (SELECT id FROM {course} WHERE format=?)", ['toggletop']);

        upgrade_plugin_savepoint(true, 2017110301, 'format', 'toggletop');
    }
