...
[![Build Status](https://travis-ci.org/gjb2048/moodle-format_toggletop.svg?branch=master)](https://travis-ci.org/gjb2048/moodle-format_toggletop)
...
===========================
The primary source for downloading this branch of the format is https://moodle.org/plugins/view.php?plugin=format_toggletop
with 'Select Moodle version:' set at 'Moodle 3.4'.

The secondary source is a tagged version with the v3.4 prefix on https://github.com/gjb2048/moodle-format_toggletop/tags

If you download from the development area - https://github.com/gjb2048/moodle-format_toggletop/ - consider that
...
https://github.com/gjb2048/moodle-format_toggletop and doing a 'Pull Request' so that the rest of the
...
============
 ...
 6. Copy 'toggletop' to '/course/format/' if you have not already done so.
...
4. In '/course/format/' move old 'toggletop' directory to a backup folder outside of Moodle.
...
2. In '/course/format/' move old 'toggletop' directory to a backup folder outside of Moodle.
3. Do not copy in the new version of 'toggletop' yet!  As this will cause the upgrade to fail.
4. Upgrade to Moodle 2.2 first - http://docs.moodle.org/22/en/Upgrading_to_Moodle_2.2.
5. After you have installed Moodle 2.2, now upgrade to Moodle 2.4 with this new toggletop -
   http://docs.moodle.org/24/en/Upgrading_to_Moodle_2.4 - but before initiating the upgrade you can copy the
   new (i.e. this) 'toggletop' folder to '/course/format'.
6. Now follow 'Upgrading from Moodle 2.2' below please.
INFO: Having no 'toggletop' folder in '/course/format' is fine as the courses that use it are not accessed and
...
1.    Put Moodle in 'Maintenance Mode' so that there are no users using it bar you as the administrator - if you have
      not already done so.
2.    In '/course/format/' move old 'toggletop' directory to a backup folder outside of Moodle - if you have not
      already done so.
3.    Copy this new 'toggletop' folder to '/course/format/'.
4.    Upgrade to Moodle 2.4 by being logged in as 'admin' and clicking on 'Home'.  If you have previously upgraded but
      'toggletop' was an old version and the upgrade failed, this should still work.
5.    Follow installation instructions above.
...
1.    In your database:
2.1   Rename the table 'format_toggletop_layout' to 'format_toggletop_settings'.
2.2   With the table 'format_toggletop_settings' change all integer types to signed if using a MySQL database.
2.3   If the table 'format_toggletop_settings' does not exist, then create it and add the following fields 
      in this order:
...
2.3.4 'layoutstructure' of type 'TINYINT(1)' type, not null, no auto increment, no zero fill with a default value
      of '1'.
2.4   With the table 'format_toggletop_settings' append three new fields of 'VARCHAR(6)' type, not null, called
      'tgfgcolour', 'tgbgcolour' and 'tgbghvrcolour' in that order with the default values of '000000', 'e2e2f2'
      and 'eeeeff' respectively.
2.5   With the table 'format_toggletop_settings' append a new field 'layoutcolumns' after the 'layoutstructure' field
...
2.5.1 'layoutcolumns' of type 'TINYINT(1)' type, not null, no auto increment, no zero fill with a default value
      of '1'.
2.6   Drop the table 'format_toggletop_cookie_cnsnt'.
...
3. In '/course/format/' remove the folder 'toggletop'.
4. In the database, remove the table 'format_toggletop_settings' along with the entry for 'format_toggletop'
   ('plugin' attribute) in the table 'config_plugins'.  If using the default prefix this will be
   'mdl_format_toggletop_settings' and 'mdl_config_plugins' respectively.
...
the 'toggle all' functionality.  For example:
    ...
    body.jsenabled .course-content ul.ctopics li.section .content .toggle-arrow span.toggle_closed {
        background-image: url([[pix:format_toggletop|arrow_right]]);
    }

    #toggle-all .content .toggle-arrow h4 span.off {
        background-image: url([[pix:format_toggletop|arrow_down]]); 
    }
...
3.  Sometimes when restoring a course, it is accessed for the first time and a toggle is clicked a 'Error updating user
    preference 'toggletop_toggle_x'' (where 'x' is the course id as shown in the URL 'id=x') can occur.  I'm not completely sure
    ...
    if you encounter it.
4.  If you get HTTP 403 errors on the browsers console for the 'settoggletoppref.php' then check that the permissions within the
    'toggletop' folder are 755 for folders and 644 for files.  Ref: https://moodle.org/mod/forum/discuss.php?d=329620.

Reporting Issues
================
Before reporting an issue, please ensure that you are running the latest version for your release of Moodle.  The primary
release area is located on https://moodle.org/plugins/view.php?plugin=format_toggletop.  It is also essential that you are
...
on Moodle tracker 'tracker.moodle.org' ensuring that you chose the 'Non-core contributed modules' and 'Course Format: toggletop'
