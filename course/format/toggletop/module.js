 ...
 * @namespace
 */
M.format_toggletop = M.format_toggletop || {};
...
M.format_toggletop.thesparezeros = "00000000000000000000000000";
M.format_toggletop.togglestate = false;
M.format_toggletop.courseid = 0;
M.format_toggletop.togglePersistence = 1; // Toggle persistence - 1 = on, 0 = off.
M.format_toggletop.ourYUI = false;
M.format_toggletop.numSections = 0;
M.format_toggletop.oneTopic = false;
M.format_toggletop.currentTopic = null; // For oneTopic when true represents the current open topic or null if none.
M.format_toggletop.currentTopicNum = false; // For oneTopic when true represents the current open topic number or 0 if none.
M.format_toggletop.userIsEditing = false;

// Namespace constants:....
M.format_toggletop.TOGGLE_6 = 1;
M.format_toggletop.TOGGLE_5 = 2;
M.format_toggletop.TOGGLE_4 = 4;
M.format_toggletop.TOGGLE_3 = 8;
M.format_toggletop.TOGGLE_2 = 16;
M.format_toggletop.TOGGLE_1 = 32;
 ...
 */
M.format_toggletop.init = function(Y, theCourseId, theToggleState, theNumSections, theTogglePersistence,
    ...
    var current = $('ul.ctopics.topics .section.main .sectionhead');
          var currentid = $(current).attr('id');
            if(currentid === 'toggle-1'){
                $("#toggle-1 span.the_toggle").addClass('toggle_open').removeClass('toggle_closed');
                $("#toggledsection-1").slideDown();
                $("#toggledsection-1").addClass('sectionopen');  
            }
...
M.format_toggletop.toggleClick = function(e) {
...
M.format_toggletop.allOpenClick = function(e) {
    e.preventDefault();
    M.format_toggletop.ourYUI.all(".toggledsection").addClass('sectionopen');
    M.format_toggletop.ourYUI.all(".toggle span.the_toggle").addClass('toggle_open').removeClass('toggle_closed')
        .setAttribute('aria-pressed', 'true');
    M.format_toggletop.resetState(M.format_toggletop.get_max_digit());
    M.format_toggletop.save_toggles();
};

M.format_toggletop.allCloseClick = function(e) {
    e.preventDefault();
    M.format_toggletop.ourYUI.all(".toggledsection").removeClass('sectionopen');
    M.format_toggletop.ourYUI.all(".toggle span.the_toggle").addClass('toggle_closed').removeClass('toggle_open')
        .setAttribute('aria-pressed', 'false');
    M.format_toggletop.resetState(M.format_toggletop.get_min_digit());
    M.format_toggletop.save_toggles();
};

M.format_toggletop.resetState = function(dchar) {
    M.format_toggletop.togglestate = "";
    var numdigits = M.format_toggletop.get_required_digits(M.format_toggletop.numSections);
    for (var i = 0; i < numdigits; i++) {
        M.format_toggletop.togglestate += dchar;
    }
};

// Toggle functions
// Args - targetNode that initiated the call, toggleNum the number of the toggle.
M.format_toggletop.toggle_topic = function(targetNode, toggleNum) {
    "use strict";
...
$.each(current, function( index, value ) {
  ...
  var currentid = 'toggle-'+toggleNum ;
  var tests = "#"+valueid+" span.the_toggle";
    if(valueid === currentid){

        if (!target.hasClass('toggle_open') && target.hasClass('toggle_closed')) {
            //alert(valueid+'equalif');
            $("#"+valueid+" span.the_toggle").addClass('toggle_open').removeClass('toggle_closed');
            $("#toggledsection-"+i).slideDown();
            $("#toggledsection-"+i).addClass('sectionopen');
            
            //$("#toggledsection-"+i).slideDown();
        } else{
            //alert(valueid+'equalelse');
            $("#"+valueid+" span.the_toggle").addClass('toggle_closed').removeClass('toggle_open');
            $("#toggledsection-"+i).slideUp();
            $("#toggledsection-"+i).removeClass('sectionopen');
        ...
        if (target.hasClass('toggle_open') && !target.hasClass('toggle_closed')) {
            //alert(valueid+'notequalif');
            $("#"+valueid+" span.the_toggle").addClass('toggle_closed').removeClass('toggle_open');
            $("#toggledsection-"+i).slideUp();
            $("#toggledsection-"+i).removeClass('sectionopen');
            
            //$("#toggledsection-"+i).slideUp();
        } else if(valueid !== currentid){
            //alert(valueid+'notequaelseif')
            $("#"+valueid+" span.the_toggle").removeClass('toggle_open').addClass('toggle_closed');
            $("#toggledsection-"+i).slideUp();
            $("#toggledsection-"+i).removeClass('sectionopen');
...
// console.log($('ul.ctopics.topics .section.main .sectionhead'));
...
M.format_toggletop.to2baseString = function(thirtysix) {
    "use strict";
...
M.format_toggletop.save_toggles = function() {
    "use strict";
    if (this.togglePersistence == 1) { // Toggle persistence - 1 = on, 0 = off.
        M.format_toggletop.set_user_preference('toggletop_toggle_' + this.courseid, this.togglestate);
    }
};

// New base 64 code:....
M.format_toggletop.is_old_preference = function(pref) {
    "use strict";
    var retr = false;
...
M.format_toggletop.convert_to_new_preference = function() {
    "use strict";
 ...
 */
M.format_toggletop.set_toggle_state = function(togglenum, state) {
    "use strict";
...
M.format_toggletop.get_required_digits = function(numtoggles) {
    "use strict";
    return this.get_toggle_pos(numtoggles);
};

M.format_toggletop.get_toggle_pos = function(togglenum) {
    "use strict";
    return Math.ceil(togglenum / 6);
};

M.format_toggletop.get_min_digit = function() {
    "use strict";
    return ':'; // Digit ':' is 58.
};

M.format_toggletop.get_max_digit = function() {
    "use strict";
    return 'y'; // Digit 'y' is 121.
};

M.format_toggletop.get_toggle_flag = function(togglenum, togglecharpos) {
    "use strict";
...
M.format_toggletop.decode_character_to_value = function(character) {
    "use strict";
    return character.charCodeAt(0) - 58;
};

M.format_toggletop.encode_value_to_character = function(val) {
    "use strict";
 ...
 */
M.format_toggletop.set_user_preference = function(name, value) {
    YUI().use('io', function(Y) {
        var url = M.cfg.wwwroot + '/course/format/toggletop/settoggletoppref.php?sesskey=' + M.cfg.sesskey + '&pref=' + encodeURI(name) + '&value=' + encodeURI(value); // jshint ignore:line
        ...
        if (M.cfg.developerdebug) {
            cfg.on.failure = function() {
                console.log("Error updating toggletop preference '" + name + "' using AJAX.  Almost certainly your session has timed out.  Clicking this link will repeat the AJAX call that failed so you can see the error: "); // jshint ignore:line
            };
        }
