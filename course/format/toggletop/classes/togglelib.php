...
class togglelib {
     ...
     */
    static public function required_toggletop_param($parname) {
        if (empty($parname)) {
            throw new coding_exception('required_toggletop_param() requires $parname to be specified');
        }
        $param = required_param($parname, PARAM_RAW);

        return self::clean_toggletop_param($param);
    }

    /**
     * Used by required_toggletop_param to clean the toggle parameter.
     *
     * @param string $param the variable we are cleaning
     * @return mixed
     * @throws coding_exception
     */
    static public function clean_toggletop_param($param) {
        if (is_array($param)) {
            throw new coding_exception('clean_toggletop_param() can not process arrays.');
        } else if (is_object($param)) {
            if (method_exists($param, '__toString')) {
                $param = $param->__toString();
            } else {
                throw new coding_exception('clean_toggletop_param() can not process objects.');
            }
        }
