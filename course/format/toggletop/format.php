...
if (!empty($displaysection)) {
    ...
    $defaulttogglepersistence = clean_param(get_config('format_toggletop', 'defaulttogglepersistence'), PARAM_INT);

    if ($defaulttogglepersistence == 1) {
        user_preference_allow_ajax_update('toggletop_toggle_' . $course->id, PARAM_RAW);
        $userpreference = get_user_preferences('toggletop_toggle_' . $course->id);
    } else {
        $userpreference = null;
    }

    $defaultuserpreference = clean_param(get_config('format_toggletop', 'defaultuserpreference'), PARAM_INT);
    ...
    echo \format_toggletop\toolbox::hex2rgba($tcsettings['togglebackgroundcolour'], $tcsettings['togglebackgroundopacity']);
    echo ';';
    echo '}';
    ...
    echo '.course-content ul.ctopics li.section .content.sectionhidden {';
    echo 'color: ';
    echo \format_toggletop\toolbox::hex2rgba($tcsettings['toggleforegroundcolour'], $tcsettings['toggleforegroundopacity']);
    echo ';';
    echo 'text-align: ';
    ...
    echo 'color: ';
    echo \format_toggletop\toolbox::hex2rgba($tcsettings['toggleforegroundhovercolour'], $tcsettings['toggleforegroundhoveropacity']);
    echo ';';
    echo '}';

    echo '.course-content ul.ctopics li.section .content div.toggle:hover {';
    echo 'background-color: ';
    echo \format_toggletop\toolbox::hex2rgba($tcsettings['togglebackgroundhovercolour'], $tcsettings['togglebackgroundhoveropacity']);
    echo ';';
    echo '}';

    $toggletopsidewidth = get_string('toggletopsidewidthlang', 'format_toggletop');
    $toggletopsidewidthdelim = strpos($toggletopsidewidth, '-');
    $toggletopsidewidthlang = strcmp(substr($toggletopsidewidth, 0, $toggletopsidewidthdelim), current_language());
    $toggletopsidewidthval = substr($toggletopsidewidth, $toggletopsidewidthdelim + 1);
    // Dynamically changing widths with language.
    if ((!$PAGE->user_is_editing()) && ($portable == 0) && ($toggletopsidewidthlang == 0)) {
        echo '.course-content ul.ctopics li.section.main .content, .course-content ul.ctopics li.tcsection .content {';
        echo 'margin: 0 '.$toggletopsidewidthval.';';
        echo '}';
    ...
    $tcborderradiustl = clean_param(get_config('format_toggletop', 'defaulttoggleborderradiustl'), PARAM_TEXT);
    $tcborderradiustr = clean_param(get_config('format_toggletop', 'defaulttoggleborderradiustr'), PARAM_TEXT);
    $tcborderradiusbr = clean_param(get_config('format_toggletop', 'defaulttoggleborderradiusbr'), PARAM_TEXT);
    $tcborderradiusbl = clean_param(get_config('format_toggletop', 'defaulttoggleborderradiusbl'), PARAM_TEXT);
...
$PAGE->requires->js('/course/format/toggletop/format.js');
