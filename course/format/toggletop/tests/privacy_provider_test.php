...
use \format_toggletop\privacy\provider;

/**
 * Privacy unit tests for the Collapsed Topics course format.
 * @group format_toggletop
 */
class format_toggletop_privacy_testcase extends \core_privacy\tests\provider_testcase {
    ...
    protected function set_up() {
        ...
        global $PAGE;
        $this->outputus = $PAGE->get_renderer('format_toggletop');
        // Ref: https://docs.moodle.org/dev/Writing_PHPUnit_tests.
        $this->course = $this->getDataGenerator()->create_course(array('format' => 'toggletop', 'numsections' => $this->numsections),
    ...
    public function test_get_metadata() {
        $items = new collection('format_toggletop');
    ...
    public function test_export_user_preferences() {
        $togglelib = new \format_toggletop\togglelib;

        $this->set_up();
        $this->setAdminUser();

        set_user_preference('toggletop_toggle_'.$this->course->id, 'FAB');
        ...
        $prefs = $writer->get_user_preferences('format_toggletop');

        $this->assertCount(1, (array) $prefs);

        $this->assertEquals('FAB', ((array)$prefs)['toggletop_toggle_'.$this->course->id]->value);

        $description = get_string('privacy:request:preference:toggle', 'format_toggletop', (object) [
            'name' => $this->course->id,
            'value' => 'FAB',
            'decoded' => $togglelib->decode_toggle_state('FAB'),
        ]);
        $this->assertEquals($description, ((array)$prefs)['toggletop_toggle_'.$this->course->id]->description);
    }
}
