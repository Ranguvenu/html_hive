...
function display_like_unlike($itemid, $likearea){
    ...
    $likeEnable = html_writer::start_span('thubmbsup',array('title'=>'Like', 'title'=>'Like', 'onclick'=>"(function(e){ require('local_ratings/ratings').updatevalues({ action: 0 , itemid: ".$itemid.", likearea : '".$likearea."'}) })(event)"));
    $likeEnable .= $likeicon;
    ...
    $unlikeEnable = html_writer::div($unlikeicon,'thubmbsdown', array('src'=>$CFG->wwwroot.'/local/ratings/pix/unlikeN.png', 'title'=>'Dislike', 'style'=>'cursor: pointer;', 'onclick'=>"(function(e){ require('local_ratings/ratings').updatevalues({ action: 1 , itemid: ".$itemid.", likearea : '".$likearea."'}) })(event)"));
...
function display_rating($itemid, $ratearea) {
    ...
    $ask_rating_data = ask_for_rating($itemid, $ratearea, $heading=NULL, $currentuserrating->rating);
    ...
    if($ask_rating_data['enroll']){
        ...
        $displayrating = $displayrating ? $displayrating : 0;
...
function can_participate($itemid, $area){
    ...
    if(!$enroll){
        $disable = 'disabled="disabled"';
        $result .= '<div>You need to enroll to the '.$area.' to give a rating</div>';
    }
...
function display_comment($itemid, $commentarea, $viewmore = true, $userid = null){
    ...
    if($participate_info['enroll']){
        ...
        $mycomment_content = "<div class='comment_information'>
                <span class='comment_label'>My Review</span> 
    ...
    if($viewmore){
        $return .= html_writer::link(new \moodle_url('/local/ratings/reviews.php',  array('itemid' => $itemid, 'commentarea' => $commentarea)), 'View All Reviews', array('class' => 'pull-right ml-1'));
    }
