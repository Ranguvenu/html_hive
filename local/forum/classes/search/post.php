...
class post extends \core_search\base_mod {
    ...
    public function get_document_recordset($modifiedfrom = 0, \context $context = null) {
        ...
        $sql = "SELECT fp.*, f.id AS forumid, f.course AS courseid
                  ...
                  JOIN {local_forum_discussions} fd ON fd.id = fp.discussion
                  JOIN {local_forum} f ON f.id = fd.forum
          $contextjoin
    ...
    public function check_access($id) {
        ...
        try {
            ...
            //$cminfo = $this->get_cm('local_forum', $forum->id, $forum->course);
        ...
        if (!local_forum_user_can_see_post($forum, $discussion, $post, $USER)) {
            return \core_search\manager::ACCESS_DENIED;
        }
    ...
    public function get_doc_url(\core_search\document $doc) {
        ...
        return new \moodle_url('/local/forum/discuss.php', array('d' => $post->discussion));
    }
    ...
    public function get_context_url(\core_search\document $doc) {
        //$contextmodule = \context::instance_by_id($doc->get('forumid'));
        return new \moodle_url('/local/forum/view.php', array('id' => $doc->get('forumid')));
    }
    ...
    protected function get_post($postid) {
        if (empty($this->postsdata[$postid])) {
            ...
            if (!$this->postsdata[$postid]) {
                throw new \dml_missing_record_exception('local_forum_posts');
            }
        }
    ...
    protected function get_forum($forumid) {
        ...
        if (empty($this->forumsdata[$forumid])) {
            $this->forumsdata[$forumid] = $DB->get_record('local_forum', array('id' => $forumid), '*', MUST_EXIST);
        }
    ...
    protected function get_discussion($discussionid) {
        ...
        if (empty($this->discussionsdata[$discussionid])) {
            $this->discussionsdata[$discussionid] = $DB->get_record('local_forum_discussions',
