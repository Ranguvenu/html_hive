...
class subscriptions {
     ...
     * @var int
     */
    const LOCAL_FORUM_DISCUSSION_UNSUBSCRIBED = -1;
    ...
    public static function is_subscribed($userid, $forum, $discussionid = null) {
        ...
        if (self::is_forcesubscribed($forum)) {
            ...
            //}
            if (has_capability('local/forum:allowforcesubscribe', \context_system::instance(), $userid)) {
                return true;
            }
        }
        ...
        if (isset($subscriptions[$discussionid])) {
            return ($subscriptions[$discussionid] != self::LOCAL_FORUM_DISCUSSION_UNSUBSCRIBED);
        }
    ...
    public static function is_forcesubscribed($forum) {
        return ($forum->forcesubscribe == LOCAL_FORUM_FORCESUBSCRIBE);
    }
    ...
    public static function subscription_disabled($forum) {
        return ($forum->forcesubscribe == LOCAL_FORUM_DISALLOWSUBSCRIBE);
    }
    ...
    public static function is_subscribable($forum) {
        return (isloggedin() && !isguestuser() &&
                !\local_forum\subscriptions::is_forcesubscribed($forum) &&
                !\local_forum\subscriptions::subscription_disabled($forum));
    }
    ...
    public static function set_subscription_mode($forumid, $status = 1) {
        global $DB;
        return $DB->set_field("local_forum", "forcesubscribe", $status, array("id" => $forumid));
    }
    ...
    public static function get_unsubscribable_forums() {
        ...
        $sql = "SELECT f.id
                FROM {local_forum} f
                LEFT JOIN {local_forum_subscriptions} fs ON (fs.forum = f.id AND fs.userid = :userid)
                WHERE f.forcesubscribe <> :forcesubscribe
                AND fs.id IS NOT NULL";
        $params = array_merge($courseparams, array(
            'userid' => $USER->id,
            'forcesubscribe' => LOCAL_FORUM_FORCESUBSCRIBE,
        ));
     ...
     */
    public static function get_potential_subscribers($context, $local_forum, $groupid, $fields, $sort = '') {
        global $DB;
        if ($groupid) {
            $cohortmembers = $DB->get_records_sql_menu('select id, userid from {cohort_members} where cohortid = ?', array($groupid));
            $cohortmemberslist = implode(',', $cohortmembers);
            if ($cohortmemberslist)
            $depsql = " u.id IN ($cohortmemberslist) ";
            else
            $depsql = 'u.id = 0'; // return none
        }        
        elseif ($local_forum->departmentid)
        // $depsql = " find_in_set (u.open_departmentid, '$local_forum->departmentid')";
        $depsql = " CONCAT(',',{$local_forum->departmentid},',') LIKE ('%,'u.open_departmentid,',%') ";
        elseif ($local_forum->costcenterid)
        $depsql = " u.open_costcenterid =  $local_forum->costcenterid";
        else
        $depsql = 'u.id = 0'; // return none
        $sql = "SELECT $fields
                FROM {user} u
        ...
        if (!isset(self::$fetchedforums[$forumid])) {
            ...
            if (isset($userid)) {
                ...
                if (!isset(self::$forumcache[$userid][$forumid])) {
                    if ($DB->record_exists('local_forum_subscriptions', array(
                        'userid' => $userid,
                        'forum' => $forumid,
                    ))) {
                ...
                $subscriptions = $DB->get_recordset('local_forum_subscriptions', array(
                    'forum' => $forumid,
                ), '', 'id, userid');
    ...
    public static function fill_subscription_cache_for_course($courseid, $userid) {
        ...
        $sql = "SELECT
                ...
                FROM {local_forum} f
                LEFT JOIN {local_forum_subscriptions} s ON (s.forum = f.id AND s.userid = :userid)
        ...
        $subscriptions = $DB->get_recordset_sql($sql, array(
            'course' => $courseid,
            'userid' => $userid,
            'subscriptionforced' => LOCAL_FORUM_FORCESUBSCRIBE,
        ));
    ...
    public static function fetch_subscribed_users($forum, $groupid = 0, $context = null, $fields = null,
        ...
        if (self::is_forcesubscribed($forum)) {
            $results = \local_forum\subscriptions::get_potential_subscribers($context, $groupid, $fields, "u.email ASC");
            ...
            if ($includediscussionsubscriptions) {
                ...
                $params['unsubscribed'] = self::LOCAL_FORUM_DISCUSSION_UNSUBSCRIBED;

                $sql = "SELECT $fields
                        FROM (
                            SELECT userid FROM {local_forum_subscriptions} s
                            WHERE
                                s.forum = :sforumid
                                UNION
                            SELECT userid FROM {local_forum_discussion_subs} ds
                            WHERE
        ...
        if (!isset(self::$discussionfetchedforums[$forumid])) {
            ...
            if (isset($userid)) {
                ...
                if (!isset(self::$forumdiscussioncache[$userid][$forumid])) {
                    $subscriptions = $DB->get_recordset('local_forum_discussion_subs', array(
                        'userid' => $userid,
                        'forum' => $forumid,
    ...
    public static function subscribe_user($userid, $forum, $context = null, $userrequest = false) {
        ...
        $sub->userid  = $userid;
        $sub->forum = $forum->id;
        $result = $DB->insert_record("local_forum_subscriptions", $sub);

        if ($userrequest) {
            $discussionsubscriptions = $DB->get_recordset('local_forum_discussion_subs', array('userid' => $userid, 'forum' => $forum->id));
            $DB->delete_records_select('local_forum_discussion_subs',
                    'userid = :userid AND forum = :forumid AND preference <> :preference', array(
                        'userid' => $userid,
                        'forumid' => $forum->id,
                        'preference' => self::LOCAL_FORUM_DISCUSSION_UNSUBSCRIBED,
                    ));
            ...
            if (isset(self::$forumdiscussioncache[$userid]) && isset(self::$forumdiscussioncache[$userid][$forum->id])) {
                foreach (self::$forumdiscussioncache[$userid][$forum->id] as $discussionid => $preference) {
                    if ($preference != self::LOCAL_FORUM_DISCUSSION_UNSUBSCRIBED) {
        ...
        $event  = \local_forum\event\subscription_created::create($params);
    ...
    public static function unsubscribe_user($userid, $forum, $context = null, $userrequest = false) {
        ...
        $DB->delete_records('local_forum_digests', $sqlparams);

        if ($forumsubscription = $DB->get_record('local_forum_subscriptions', $sqlparams)) {
            $DB->delete_records('local_forum_subscriptions', array('id' => $forumsubscription->id));

            if ($userrequest) {
                $discussionsubscriptions = $DB->get_recordset('local_forum_discussion_subs', $sqlparams);
                $DB->delete_records('local_forum_discussion_subs',
                        array('userid' => $userid, 'forum' => $forum->id, 'preference' => self::LOCAL_FORUM_DISCUSSION_UNSUBSCRIBED));
            ...
            $event = \local_forum\event\subscription_deleted::create($params);
    ...
    public static function subscribe_user_to_discussion($userid, $discussion, $context = null) {
        ...
        $subscription = $DB->get_record('local_forum_discussion_subs', array('userid' => $userid, 'discussion' => $discussion->id));
        if ($subscription) {
            if ($subscription->preference != self::LOCAL_FORUM_DISCUSSION_UNSUBSCRIBED) {
        ...
        if ($DB->record_exists('local_forum_subscriptions', array('userid' => $userid, 'forum' => $discussion->forum))) {
            if ($subscription && $subscription->preference == self::LOCAL_FORUM_DISCUSSION_UNSUBSCRIBED) {
                // The user is subscribed to the forum, but unsubscribed from the discussion, delete the discussion preference.
                $DB->delete_records('local_forum_discussion_subs', array('id' => $subscription->id));
            ...
            if ($subscription) {
                ...
                $DB->update_record('local_forum_discussion_subs', $subscription);
            } else {
                ...
                $subscription->id = $DB->insert_record('local_forum_discussion_subs', $subscription);
                self::$forumdiscussioncache[$userid][$discussion->forum][$discussion->id] = $subscription->preference;
            }
        }

        //$context = local_forum_get_context($discussion->forum, $context);
        //$params = array(
        //    'context' => $context,
    ...
    public static function unsubscribe_user_from_discussion($userid, $discussion, $context = null) {
        ...
        if (!$DB->record_exists('local_forum_subscriptions', array('userid' => $userid, 'forum' => $discussion->forum))) {
            ...
            if ($subscription) {
                $subscription->preference = self::LOCAL_FORUM_DISCUSSION_UNSUBSCRIBED;
                $DB->update_record('local_forum_discussion_subs', $subscription);
            } else {
                $subscription = new \stdClass();
                $subscription->userid  = $userid;
