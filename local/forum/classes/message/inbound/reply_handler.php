...
namespace local_forum\message\inbound;
...
class reply_handler extends \core\message\inbound\handler {
    ...
    public function get_description() {
        return get_string('reply_handler', 'local_forum');
    }
    ...
    public function get_name() {
        return get_string('reply_handler_name', 'local_forum');
    }
    ...
    public function process_message(\stdClass $record, \stdClass $messagedata) {
        ...
        $post = $DB->get_record('local_forum_posts', array('id' => $record->datavalue));
        if (!$post) {
        ...
        $forum = $DB->get_record('local_forum', array('id' => $discussion->forum));
        ...
        if (!$canpost) {
            $data = new \stdClass();
            $data->forum = $forum;
            throw new \core\message\inbound\processing_failed_exception('messageinboundnopostforum', 'local_forum', $data);
        }
        ...
        if (!empty($thresholdwarning) && !$thresholdwarning->canpost) {
            ...
            throw new \core\message\inbound\processing_failed_exception('messageinboundthresholdhit', 'local_forum', $data);
        }

        $subject = clean_param($messagedata->envelope->subject, PARAM_TEXT);
        $restring = get_string('re', 'local_forum');
        ...
        if (!empty($messagedata->attachments['attachment']) && count($messagedata->attachments['attachment'])) {
            ...
            if (!forum_can_create_attachment($forum, $modcontext)) {
                ...
                throw new \core\message\inbound\processing_failed_exception('messageinboundattachmentdisallowed', 'local_forum', $data);
            }
            ...
            if ($forum->maxattachments < $attachmentcount) {
                ...
                $data->forum = $forum;
                $data->attachmentcount = $attachmentcount;
                throw new \core\message\inbound\processing_failed_exception('messageinboundfilecountexceeded', 'local_forum', $data);
            }
            ...
            if ($forum->maxbytes < $filesize) {
                ...
                throw new \core\message\inbound\processing_failed_exception('messageinboundfilesizeexceeded', 'local_forum', $data);
            }
        }
        ...
        $event = \local_forum\event\post_created::create($params);
    ...
    public function get_success_message(\stdClass $messagedata, $handlerresult) {
        ...
        $discussionurl = new \moodle_url('/local/forum/discuss.php', array('d' => $handlerresult->discussion));
        $discussionurl->set_anchor('p' . $handlerresult->id);
        $a->discussionurl = $discussionurl->out();

        $message = new \stdClass();
        $message->plain = get_string('postbymailsuccess', 'local_forum', $a);
        $message->html = get_string('postbymailsuccess_html', 'local_forum', $a);
        return $message;
    }
}
