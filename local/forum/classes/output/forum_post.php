...
class forum_post implements \renderable, \templatable {
     ...
     */
    public function __construct($forum, $discussion, $post, $author, $recipient, $canreply) {
        //$this->course = $course;
        //$this->cm = $cm;
        $this->forum = $forum;
     ...
     *
     * @param \local_forum_renderer $renderer The render to be used for formatting the message and attachments
     ...
     * @return stdClass Data ready for use in a mustache template
     */
    protected function export_for_template_text(\local_forum_renderer $renderer) {
        return array(
            ...
            'message'                       => html_entity_decode($renderer->format_message_text($this->post)),
            'attachments'                   => html_entity_decode($renderer->format_message_attachments($this->post)),
     ...
     * @return stdClass Data ready for use in a mustache template
     */
    protected function export_for_template_html(\local_forum_renderer $renderer) {
        return array(
            'id'                            => $this->post->id,
            ...
            // Format some components according to the renderer.
            'message'                       => $renderer->format_message_text($this->post),
            'attachments'                   => $renderer->format_message_attachments($this->post),
    ...
    public function get_unsubscribeforumlink() {
        if (!\local_forum\subscriptions::is_subscribable($this->forum)) {
            return null;
        }
        $link = new \moodle_url(
            '/local/forum/subscribe.php', array(
                'id' => $this->forum->id,
            )
        );
    ...
    public function get_postdate() {
        ...
        if (!empty($CFG->local_forum_enabletimedposts) && ($this->discussion->timestart > $postmodified)) {
    ...
    public function get_author_picture() {
        ...
        return $OUTPUT->user_picture($this->author, array('courseid' => 1));
    }
