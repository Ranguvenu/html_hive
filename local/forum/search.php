...
//);
//
//$event = \local_forum\event\course_searched::create($params);
//$event->trigger();

$strforums = get_string("modulenameplural", "local_forum");
$strsearch = get_string("search", "local_forum");
$strsearchresults = get_string("searchresults", "local_forum");
$strpage = get_string("page");

if (!$search || $showform) {

    $PAGE->navbar->add($strforums, new moodle_url('/local/forum/index.php', array('id'=>$id)));
    $PAGE->navbar->add(get_string('advancedsearch', 'local_forum'));

    $PAGE->set_title($strsearch);
    $PAGE->set_heading($course->fullname);
    echo $OUTPUT->header();

    local_forum_print_big_search_form($course);
    echo $OUTPUT->footer();
    exit;
}
...
$searchform = local_forum_search_form($course, $search);

$PAGE->navbar->add($strsearch, new moodle_url('/local/forum/search.php', array('id'=>$forumid)));
$PAGE->navbar->add($strsearchresults);
if (!$posts = local_forum_search_posts($searchterms, $forumid, $page*$perpage, $perpage, $totalcount)) {
    $PAGE->set_title($strsearchresults);
    $PAGE->set_heading($course->fullname);
    echo $OUTPUT->header();
    echo $OUTPUT->heading($strforums, 2);
    echo $OUTPUT->heading($strsearchresults, 3);
    echo $OUTPUT->heading(get_string("noposts", "local_forum"), 4);
...
$url    = new moodle_url("/local/forum/search.php", $params);
/* foreach ($tags as $tag) {
    $url .= "&tags[]=$tag";
} */
echo html_writer::link($url, get_string('advancedsearch', 'local_forum').'...');
...
foreach ($posts as $post) {
    ...
    if ($missing_terms) {
        $strmissingsearchterms = get_string('missingsearchterms','local_forum');
    ...
    $fulllink = "<a href=\"discuss.php?d=$post->discussion#p$post->id\">".get_string("postincontext", "local_forum")."</a>";
    ...
    // Now pring the post.
    local_forum_print_post($post, $discussion, $forum, false, false, false, $fulllink, '', -99, false);
}
...
echo $OUTPUT->footer();
  ...
  */
function local_forum_print_big_search_form($course) {
...
function forum_clean_search_terms($words, $prefix='') {
  ...
  */
function local_forum_menu_list($course)  {
    $menu = array();
