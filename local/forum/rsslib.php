...
function forum_rss_get_feed($context, $args) {
    ...
    if ($context->id != $modcontext->id || !has_capability('local/forum:viewdiscussion', $modcontext)) {
        return null;
    }
...
function forum_rss_feed_discussions_sql($forum, $cm, $newsince=0) {
    ...
    if (!empty($CFG->forum_enabletimedposts)) { /// Users must fulfill timed posts
        if (!has_capability('local/forum:viewhiddentimedposts', $modcontext)) {
...
function forum_rss_feed_contents($forum, $sql, $params, $context) {
    ...
    foreach ($recs as $rec) {
            ...
            if ($isdiscussion) {
                $item->link = $CFG->wwwroot."/local/forum/discuss.php?d=".$rec->discussionid;
            } else {
                $item->link = $CFG->wwwroot."/local/forum/discuss.php?d=".$rec->discussionid."&parent=".$rec->postid;
            }
    ...
    $header = rss_standard_header(\local_costcenter\lib::strip_tags_custom(format_string($forum->name,true)),
                                  $CFG->wwwroot."/local/forum/view.php?f=".$forum->id,
