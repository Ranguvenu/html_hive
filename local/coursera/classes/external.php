...
use local_coursera\output\courseraview;

class local_coursera_external  extends external_api{
    /**
     * Returns description of method parameters.
     *
     * @return external_function_parameters.
     */
    public static function managecourseralog_parameters() {
     ...
     */
    public static function managecourseralog(
        $options,
        $dataoptions,
        $offset = 0,
        $limit = 0,
        ...
        require_login();
        $PAGE->set_url('/local/coursera/sync_history.php', array());
        $PAGE->set_context($contextid);
        // Parameter validation.
        $params = self::validate_parameters(
            self::managecourseralog_parameters(),
            [
                'options' => $options,
        ...
        $stable = new \stdClass();
        $stable->thead = true;
        $totalsynchistory = (new courseraview)::manage_courserahistory_count($stable,$filtervalues);
        $totalsynchistorycount = $totalsynchistory['count'];

        $stable->thead = false;
        $stable->start = $offset;
        $stable->length = $limit;
        $data = (new courseraview)::manage_courserahistory_content($stable,$filtervalues);
     ...
     */
    public static function  managecourseralog_returns() {
     ...
     * @return external_function_parameters.
     */
    public static function managecourserastatics_parameters() {
        return new external_function_parameters([
                'options' => new external_value(PARAM_RAW, 'The paging data for the service'),
                'dataoptions' => new external_value(PARAM_RAW, 'The data for the service'),
                'offset' => new external_value(PARAM_INT, 'Number of items to skip from the begging of the result set',
     ...
     * @return array The list of users and total users count.
     */
    public static function managecourserastatics(
        $options,
        $dataoptions,
        $offset = 0,
        $limit = 0,
        ...
        global $OUTPUT, $CFG, $DB,$USER,$PAGE;
        require_login();
        $PAGE->set_url('/local/coursera/sync_historystatistics.php', array());
        $PAGE->set_context($contextid);
        // Parameter validation.
        $params = self::validate_parameters(
            self::managecourserastatics_parameters(),
            [
                'options' => $options,
                'dataoptions' => $dataoptions,
                'offset' => $offset,
        ...
        $stable = new \stdClass();
        $stable->thead = true;
        $totalsynchistorystatistics = (new courseraview)::manage_synchistorystatistics_count($stable,$filtervalues);
        $totalsynchistorystatisticscount = $totalsynchistorystatistics['count'] ? $totalsynchistorystatistics['count'] : 0;

        $stable->thead = false;
        $stable->start = $offset;
        $stable->length = $limit;
        $data = (new courseraview)::manage_synchistorystatistics_content($stable,$filtervalues);
        return [
     ...
     * Returns description of method result value.
     */
    public static function  managecourserastatics_returns() {
        return new external_single_structure([
            'options' => new external_value(PARAM_RAW, 'The paging data for the service'),
            'dataoptions' => new external_value(PARAM_RAW, 'The data for the service'),
