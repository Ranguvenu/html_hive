 ...
 class notification{
    ...
    public function send_evaluation_notification($evaluationinstance, $touser, $fromuser, $emailtype, $notification){
    	...
    	if($evaluationinstance->timeopen && $evaluationinstance->timeclose){
            $datamailobject->feedback_schedule = date('d-m-Y',$evaluationinstance->timeopen).' To '.date('d-m-Y',$evaluationinstance->timeclose);
        }else{
    	...
    	$datamailobject->feedback_enroldate = date('d-m-Y', $enroldate);

        if($emailtype == "feedback_unenrollment"){
            $datamailobject->feedback_unenroldate = date('d-m-Y');
        }

    	$datamailobject->feedback_username = $touser->firstname;
    	...
    	if($emailtype){
            ...
            if ($completedon) {
                $completedon = date('d-m-Y', $completedon);
            } else {
                $completedon = 'N/A';
            }
