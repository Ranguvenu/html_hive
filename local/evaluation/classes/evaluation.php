...
class evaluation {
    public function evaluations_by_status($status = 'inprogress', $mobile = false, $plugin = 'site', $search = '', $page = 0, $perpage = 10, $id = 0) {
        global $DB,$USER;
        if ($status == 'inprogress') {
            ...
            $sqlquery = "SELECT e.*, eu.creatorid, eu.timemodified as joinedate";
            $sqlcount = "SELECT COUNT(DISTINCT e.id) ";
            $sql = " FROM {local_evaluations} e
                     JOIN {local_evaluation_users} eu ON eu.evaluationid = e.id
                     WHERE e.plugin = '{$plugin}' AND eu.userid = {$USER->id} AND e.evaluationmode LIKE 'SE' AND e.deleted != 1  ";
            if (!empty($search)) {
