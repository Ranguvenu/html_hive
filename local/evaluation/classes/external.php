...
class local_evaluation_external extends external_api {
    ...
    public static function submit_create_evaluation_form($contextid, $jsonformdata) {
        ...
        if ($validateddata) {
            if($validateddata->timeclose){
                $validateddata->timeclose = $validateddata->timeclose + 86399;
            }
    ...
    */
    ...
    public static function addnew_question($contextid,$id, $jsonformdata, $params){
        ...
        // pagebreak =0;
        // default =0;
        // captcha if (empty($CFG->recaptchaprivatekey) OR empty($CFG->recaptchapublickey)) {
        //     return 0;
        // }
        // return 1;
        ...
        if(file_exists($CFG->dirroot.'/local/evaluation/item/'.$data->typ.'/'.$data->typ.'_form.php')){
            ...
            //     $value->value = $data->presentation;
            // $presentation = $eval_item_obj->get_printval($data, $value);
            // print_object($presentation);
        }
        // $data->presentation = '';

        // require_once($CFG->dirroot.'/local/evaluation/item/info/lib.php');
        // $evaluationclass = new evaluation_item_info();
        // $evaluationclass->save_item();
        if (isset($data->clone_item) AND $data->clone_item) {
            $data->id = ''; //to clone this item
            $data->position++;
        }

        // $data->hasvalue = $this->get_hasvalue();
        if (!$data->id) {
    ...
    public static function evaluations_by_status_parameters() {
        return new external_function_parameters(
            array(
                ...
                'perpage' => new external_value(PARAM_INT, 'Per Page', VALUE_OPTIONAL, 10),
            )
        );
    }

    public static function evaluations_by_status($status, $plugin = 'site', $search = '', $page = 0, $perpage = 10) {
        global $CFG, $DB, $USER;
     ...
     * @since Moodle 3.3
     */
    public static function get_evaluations($id, $plugin = 'site') {
        global $PAGE;
        ...
        if (!empty($params['id'])) {
            list($evaluations, $count) = (new local_evaluation\evaluation)::evaluations_by_status('enrolled', true, $plugin, '', 0, 0, $id);
        }
