...
function evaluation_add_instance($evaluation) {
    ...
    if ($evaluation->instance == 0) {
        // if (empty($evaluation->costcenterid[0])) {
        //     // $evaluation->costcenterid = get_mydepartment();
        //     $evaluation->costcenterid = $USER->open_costcenterid;
        // } else {
    ...
    // if (isset($evaluation->tags) && !empty($tagsplugin)) {
    //     local_tags_tag::set_item_tags('local_evaluation', 'evaluation', $evaluation->id, context_system::instance(), $evaluation->tags, 0, $evaluation->costcenterid, $evaluation->departmentid);
    // }
...
function evaluation_delete_instance($id) {
    ...
    if (is_array($evaluationitems)) {
        ...
        // if ($delitems = $DB->get_records("local_evaluation_item", array("evaluation"=>$id))) {
    ...
    // deleting enrolled users to the evaluation
    // $DB->delete_records('local_evaluation_users', array('evaluationid'=>$id));
    ...
    // return $DB->delete_records("local_evaluations", array("id"=>$id));
...
function evaluation_set_events($evaluation) {
    ...
    if (isset($evaluation->timeopen) && $evaluation->timeopen > 0) {
        ...
        //$event->description  = format_module_intro('local_evaluations', $evaluation, $evaluation->id);
...
function local_evaluation_quicklink_node(){
    ...
    if (is_siteadmin() || has_capability('local/evaluation:view',$systemcontext)) {
            ...
            // $local_evaluations_content = $PAGE->requires->js_call_amd('local_evaluation/newevaluation', 'init', array('[data-action=createevaluationmodal]', $systemcontext->id, $evalid, $classid, $eval_plugin));

            // $local_evaluations_content .= '<span class="anch_span"><i class="fa fa-clipboard" aria-hidden="true"></i></span>';
            // $display_line = false;
            // $local_evaluations_content .= '<div class="quick_navigation_detail">
            //                                     <div class="span_str">'.get_string("manage_br_evaluation", "local_evaluation").'</div>';
            // if(has_capability('local/evaluation:addinstance', $systemcontext) ||  is_siteadmin()){
            //     $local_evaluations_content .= '<span class="span_createlink">
            //                                         <a href="javascript:void(0);" class="quick_nav_link goto_local_evaluations" data-action="createevaluationmodal" title="'.get_string("createevaluation", "local_evaluation").'">'.get_string("create").'</a>';
            //     $display_line = true;
            // }

            // if($display_line){
            //      $local_evaluations_content .= " | ";
            // }

            // $local_evaluations_content .= '<a href="'.$CFG->wwwroot.'/local/evaluation/index.php" class="viewlink" title= "'.get_string("view_feedbacks", "local_evaluation").'">'.get_string('view').'</a>';

            // $local_evaluations_content .= '</span>';

            // $local_evaluations_content .= '</div>';
            // $local_evaluations = '<div class="quick_nav_list manage_feedbacks one_of_three_columns" >'.$local_evaluations_content.'</div>';
        $evaldata = array();
...
function costcenterwise_evaluation_count($costcenter, $department = false){
    ...
    return array('dept_feedback' => $countfeedback,'dept_feedbackactive' => $activefeedback,'dept_feedbackinactive' => $inactivefeedback);
}
...
function get_listof_evalautions($stable, $filtervalues){
    ...
    foreach($records as $record) {
        $line = array();
        $localcontext = context_system::instance();
        ...
        if($record->timeopen == 0 AND $record->timeclose == 0) {
           ...
           $dates = date("d-M-Y", $record->timeopen).  ' - '  .date("d-M-Y", $record->timeclose);
        }
...
function local_evaluation_output_fragment_enrolledusers($args) {
    ...
    if ($record->type == 1) {
            ...
            foreach($assignedusers as $assigneduser){
                ...
                if($user){
                    ...
                    if ($completed_status) {
                        ...
                        $submitteddate = date('d-m-Y', $completed_status->timemodified);
                    } else {
       ...
       // $sql ="SELECT distinct(u.id) as userid, u.firstname,u.lastname,u.email, e.id as evaluationid, eu.timecreated
       //                 from {local_evaluation_users} eu
       //                 JOIN {local_evaluations} e ON eu.evaluationid = e.id
       //                 JOIN {local_evaluation_completed} ec ON ec.evaluation = e.id
       //                 JOIN {user} u ON ec.userid = u.id AND u.deleted = 0 AND u.suspended = 0
       //                 where e.id = ?  ";
        ...
        if(!empty($assignedusers)){
            foreach($assignedusers as $assigneduser){
                ...
                if($user){
                    ...
                    // if ($completed_status) {
                        $status = get_string('completed', 'local_evaluation');
                        $submitteddate = date('d-m-Y', $assigneduser->completedon);
                    // } else {
                    //     $status = get_string('pending', 'local_evaluation');
                    //     $submitteddate = '-';
                    // }
...
function local_evaluation_output_fragment_addnew_question($args){
    ...
    $output = '';
    // $output = '<div class="tab_addedit"><h5>Add / Edit questions</h5></div><hr>';
