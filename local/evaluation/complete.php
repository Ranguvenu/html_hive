...
if(!($DB->record_exists_sql($usersql, $paramsuser) || $DB->record_exists_sql($superusersql, $paramssuperuser))){
  ...
  print_error("You dont have permission to view this page.");// hidden feedback
}
...
if (!$evaluationcompletion->is_open()) {
  ...
  if (!empty($evaluation->timeopen) AND !empty($evaluation->timeclose)) {
    $dates = date('d-M-Y H:i', $evaluation->timeopen).  ' <b>to</b> '  .date('d-M-Y H:i', $evaluation->timeclose);
    echo $OUTPUT->notification(get_string('evaluation_is_not_open', 'local_evaluation'). '. Available from <b>'.$dates.'</b>');
  } elseif(!empty($evaluation->timeopen) AND empty($evaluation->timeclose)) {
    $dates = date('d-M-Y H:i', $evaluation->timeopen);
    echo $OUTPUT->notification(get_string('evaluation_is_not_open', 'local_evaluation'). '. Available from <b>'.$dates.'</b>');
  } elseif (empty($evaluation->timeopen) AND !empty($evaluation->timeclose)) {
    $dates = date('d-M-Y H:i', $evaluation->timeclose);
    echo $OUTPUT->notification(get_string('evaluation_is_not_open', 'local_evaluation'). '. Closed on <b>'.$dates.'</b>');
  }    
...
if (!$evaluationcompletion->is_empty() && $cansubmit) {
  ...
  // $urltogo = $evaluationcompletion->process_page($gopage, $gopreviouspage, $mode);
...
if ($evaluationcompletion->is_empty()) {
  //\core\notification::error(get_string('no_items_available_yet', 'local_evaluation'));
  echo '<p align="center">';
  echo $OUTPUT->box_start('generalbox boxaligncenter');
  ...
  if ($evaluationcompletion->just_completed()) {
    ...
    // $email_logs = $emaillogs->feedback_emaillogs($type,$dataobj,$touserid,$fromuserid);
    ...
    // if((!$DB->record_exists('local_evaluation_completed',  array ('userid' => $teamuserid, 'evaluation' => $id)) && $evaluation->evaluationtype == 3) || empty($evaluation->evaluationtype)){
    if((!$DB->record_exists('local_evaluation_completed',  array ('userid' => $teamuserid, 'evaluation' => $id)))){
        ...
        echo "<div class='alert alert-info'>You have already submitted the feedback for the user <b>". fullname($userrecord)."</b></div>";
      }
  }
} else {
  echo $OUTPUT->box_start('generalbox boxaligncenter');
