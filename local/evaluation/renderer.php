...
class local_evaluation_renderer extends plugin_renderer_base  {
  ...
  public function get_evaluation_top_buttons($id, $context, $record){
   ...
   /*     if(is_siteadmin() || has_capability('local/costcenter:manage_ownorganization', $context) || has_capability('local/evaluation:create_update_question', $context)){
          $buttons[] = html_writer::start_tag('li', array('')).
                html_writer::link('javascript:void(0)', $this->pix_icon('i/edit', get_string('le_createnewquestion', 'local_evaluation'), 'moodle', array('class' => 'iconsmall', 'title' => '', 'target'=>'_blank')), array('data-fg' => 'c', 'data-method' => 'addnew_question', 'data-plugin' => 'local_evaluation', 'class' => 'course_extended_menu_itemlink', 'data-id' => $id)).
                html_writer::end_tag('li');
        }*/
        if(is_siteadmin() || has_capability('local/costcenter:manage_ownorganization', $context) || has_capability('local/evaluation:edititems', $context)){
            ...
            // $backupimg = $this->pix_icon('t/backup', get_string('export_questions', 'local_evaluation'), 'moodle', array('class' => 'iconsmall', 'title' => '', 'target'=>'_blank'));
  ...
  */
  ...
  function display($id, $context, $evaluationstructure) {
    ...
    //   $buttons[] = html_writer::link("javascript:void(0)",$OUTPUT->pix_icon('t/edit', get_string('edit'), 'moodle', array('class' => 'iconsmall', 'title' => '')), array('class'=>'course_extended_menu_itemlink', 'data-action'=>"createevaluationmodal", 'data-value'=>$record->id)); 

    //   if (has_capability('local/evaluation:viewreports', $context))
    //   $buttons[] = html_writer::link(new moodle_url('/local/evaluation/analysis.php', array('id' => $record->id, 'sesskey' => sesskey())), $OUTPUT->pix_icon('i/grades', get_string('overview', 'local_evaluation'), 'moodle', array('class' => 'iconsmall ', 'title' => '')),  array('class'=>'course_extended_menu_itemlink'));

    //   if (has_capability('local/evaluation:viewanalysepage', $context))
    //   $buttons[] = html_writer::link(new moodle_url('/local/evaluation/show_entries.php', array('id' => $record->id, 'sesskey' => sesskey())), $OUTPUT->pix_icon('i/preview', get_string('responses', 'local_evaluation'), 'moodle', array('class' => 'iconsmall', 'title' => '')),  array('class'=>'course_extended_menu_itemlink'));

    //   if (has_capability('local/evaluation:enroll_users', $context))
    //   $buttons[] = html_writer::link(new moodle_url('/local/evaluation/users_assign.php', array('id' => $record->id, 'sesskey' => sesskey())), $OUTPUT->pix_icon('i/assignroles', get_string('assignusers', 'local_evaluation'), 'moodle', array('class' => 'iconsmall', 'title' => '', 'target'=>'_blank')),  array('class'=>'course_extended_menu_itemlink'));
    ...
    //    array('id' => 'deleteconfirm' . $record->id . '', 'class'=>'course_extended_menu_itemlink', 'onclick' => '(
    //        function(e){
    //        require("local_evaluation/newevaluation").deleteevaluation("' . $record->id . '")
    //        })(event)'));
    //   }
    ...
    if (is_siteadmin() OR has_capability('local/evaluation:edititems', $context)) {
         ...
         // $line['description'] = file_prepare_draft_area($draftitemid, $context->id,
    ...
    //$data .= $this->render_from_template('local_evaluation/evaluations_list', $line);
  ...
  */
  function editquestions($id, $context, $evaluationstructure) {
      ...
      // $output.= '<lable for="type">Add question</lable><select name="type" class="target custom-select ml-15" id="id_questiontyp" value='.$id.'>
      ...
      // if (has_capability('local/evaluation:edititems', $context)) {
      //      $output .= '</br><div class="tab_addedit"><h5>Export / Import Questions</h5></div><hr>';
      //     $urlparams = array('action'=>'exportfile', 'id'=>$id);
  ...
  */
  public function get_evaluations($filter = false) {
      $systemcontext = context_system::instance();
      ...
      $options = array('targetID' => 'manage_feedbacks','perPage' => 8, 'cardClass' => 'col-lg-3 col-md-6 col-12', 'viewType' => 'card' );
      $options['methodName']='local_evaluation_evaluationview';
      $options['templateName']='local_evaluation/evaluations_list';
  ...
  public function tagged_evaluations($tagid, $exclusivemode, $ctx, $rec, $displayoptions, $count = 0, $sort='') {
    ...
    public function get_userdashboard_evaluation($tab, $filter = false){
        $systemcontext = context_system::instance();

        $options = array('targetID' => 'dashboard_evaluation', 'perPage' => 6, 'cardClass' => 'col-md-6 col-12', 'viewType' => 'card');
        $options['methodName'] = 'local_evaluation_userdashboard_content_paginated';
        $options['templateName'] = 'local_evaluation/userdashboard_paginated';
        $options['filter'] = $tab;
        $options = json_encode($options);
        $filterdata = json_encode(array());
        $dataoptions = json_encode(array('contextid' => $systemcontext->id));
