...
class user{
    public function user_profile_content($id,$return = false,$start =0,$limit=5){
        ...
        foreach($userclassrooms['data'] as $classroom){
            ...
            $classroomsummary = strip_tags($classroom->description);
            ...
            $coursefileurl  = $classroominclude->get_classroom_summary_file($classroom);
            $classroomsarray['module_img_url'] = $coursefileurl->out();
            
            /*   $classroomsarray['module_img_url'] = ((new \local_classroom\classroom)->classroom_logo($classroom->module_logo));
            
            if($classroomsarray['module_img_url'] == 0){
                require_once($CFG->dirroot.'/local/includes.php');
                $includes = new \user_course_details();
                $classroominclude = new \local_classroom\includes();
                $classroomsarray['module_img_url']   = $classroominclude->get_classroom_summary_file($classroom);
               // $classroomsarray['module_img_url'] = ($includes->get_classes_summary_files($classroom))->out();
            }else{
                $classroomsarray['module_img_url'] = $classroomsarray['background_logourl']->out();
            } */
    ...
    public function user_modulewise_content($id,$start =0,$limit=5){
      ...
      foreach($userclassrooms['data'] as $classroom){
          ...
          $classroomsarray['enrolldate'] = date('d-m-Y H:i:s',$classroom->enrolldate);
          ...
          if(!empty($classroom->completiondate)){
            $classroomsarray['completiondate'] = date('d-m-Y H:i',$classroom->completiondate);
          }else{
