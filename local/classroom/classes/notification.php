...
class notification{
    ...
    public function classroom_feedback_reminder($emailtype, $touser, $fromuser, $classroominstance,$waitinglistid=0){
        if($notification = $this->get_existing_notification($classroominstance, $emailtype)){
            $this->send_classroom_notification($classroominstance, $touser, $fromuser, $emailtype, $notification,$waitinglistid);
        }
        $this->mobile_notifications($emailtype, $classroominstance, $touser, $fromuser);
	...
	public function send_classroom_notification($classroominstance, $touser, $fromuser, $emailtype, $notification,$waitinglistid=0){
        ...
        $datamailobject->classroom_startdate = $classroominstance->startdate ? date('d-m-Y',$classroominstance->startdate) :'N/A';
        $datamailobject->classroom_enddate = $classroominstance->enddate ? date('d-m-Y',$classroominstance->enddate) :'N/A';
        ...
        $classroomasignedrolename=$this->get_rolename_inclassroom($classroominstance, $touser);
        $datamailobject->classroom_asigned_rolename = $classroomasignedrolename ? $classroomasignedrolename :'N/A';


        $datamailobject->classroom_enroluserfulname = $touser ? $touser->firstname :'N/A';
        ...
        if(in_array($emailtype, $completiondatestring)){
            ...
            $datamailobject->classroom_completiondate = $completiondate ? date('d-m-Y', $completiondate) : 'N/A';
        }
        if(in_array($emailtype, array('classroom_feedback_reminder'))){
            $datamailobject->feedback_name = $classroominstance->feedback_name;
            $datamailobject->classroom_feedbackurl = $CFG->wwwroot .'/'.$classroominstance->classroom_feedbackurl;
        }
