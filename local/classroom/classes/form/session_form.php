...
class session_form extends moodleform {
    ...
    public function definition() {
        ...
        $trainers = array();
        $select_trainer = array(null=>get_string('select_trainers','local_classroom'));
        ...
        if (!empty($trainerid)) {
            $trainers =  $select_trainer + $DB->get_records_menu('user', array('id' => $trainerid), '',
                "id, CONCAT(firstname, ' ' , lastname) AS fullname");
        } else {
            $trainers = $select_trainer;
        }
    ...
    public function validation($data, $files) {
		...
		if(isset($data['name']) &&empty(trim($data['name']))){
            $errors['name'] = get_string('valsessionnamerequired','local_classroom');
        }
        return $errors;
    }
}