...
class online_sessions_integration{
    ...
    public function classroom_module_integration($instance_type,$session,$new_session,$type){
	 ...
	 if($instance_type =='zoom'){       
	 require_once($CFG->dirroot . '/mod/zoom/locallib.php');
		      
              $online_module->host_id = zoom_get_user_id();
              $online_module->start_time=$session->timestart;
			  $online_module->duration =($session->timefinish - $session->timestart);
