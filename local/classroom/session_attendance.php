...
$PAGE->set_url('/local/classroom/session_attendance.php');
$STD_FIELDS = array('employee_id','employee_email','attendance_status');
...
$mform = new local_classroom\form\uploadsessionattendance_form(null,array('cid' => $cid, 'sid' => $sid));
...
if ($formdata = $mform->get_data()) {
    ...
    $hrms = new local_classroom\upload\session_attendance();
    $hrms->session_attendanceupload($cir, $filecolumns, $formdata);

}
else{
    echo $OUTPUT->header();
    
    $classroom_backurl = new moodle_url('/local/classroom/index.php');
    ...
    echo html_writer::link(new moodle_url('/local/classroom/uploadattendancesample.php?format=csv'),get_string('sample','local_classroom'),array('id'=>'classroom_attendance_upload'));
    echo html_writer::link(new moodle_url('/local/classroom/attendance_help.php'),get_string('help_manual','local_classroom') ,array('id'=>'classroom_attendance_upload','target'=>'__blank'));
