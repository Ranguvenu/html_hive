...
if ($hassiteconfig) {
    ...
    $input = array(null=>new lang_string('enter_userid', 'local_classroom'));
    foreach (core_component::get_plugin_list('mod') as $type => $notused) {
              
        if($type=='wiziq'||$type=='bigbluebuttonbn'||$type=='webexactivity' ||$type=='zoom' ){
    ...
    $name = new lang_string('outlook_event_credential', 'local_classroom');
    $description = new lang_string('outlook_event_credential_desc', 'local_classroom');
    $settings->add(new admin_setting_configtext('local_classroom/outlook_event_credential', get_string('outlook_event_credential', 'local_classroom'),
                '', '2', PARAM_INT)); 
