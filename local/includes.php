...
class user_course_details {
	...
	function total_course_activities($courseid) {
		...
		$sql="SELECT COUNT(ccc.id) as totalactivities FROM {course_completion_criteria} ccc WHERE ccc.course = {$courseid}"; 
	...
	function user_course_completed_activities($courseid, $userid) {
		...
		$sql = "SELECT COUNT(cc.id) as completedact FROM {course_modules_completion} cc JOIN {course_completion_criteria} ccc ON cc.coursemoduleid = ccc.moduleinstance WHERE ccc.course = $courseid AND cc.userid = $userid AND cc.completionstate = 1";
...
/*  	function user_course_completion_progress($courseid, $userid) {
	...
	function course_summary_files($courserecord){
		...
		if(empty($url)){		
			...
			$coursetypeimage = $DB->get_field('local_course_types','course_image',array('id'=>$courserecord->open_identifiedas));      
			$url = course_img_path($coursetypeimage);						
		}		
	 	if(empty($url)){
			$url = $OUTPUT->image_url('courseimg', 'local_courses');//send_file_not_found();
		} 
