...
$PAGE->set_heading(get_string('uploaduploadcredits', 'local_courses'));
$strheading = get_string('pluginname', 'local_courses') . ' : ' . get_string('uploadcredits', 'local_courses');

$PAGE->set_title($strheading);
$PAGE->navbar->add(get_string('pluginname', 'local_courses'), new moodle_url('/local/courses/courses.php'));
$PAGE->navbar->add(get_string('uploadcredits', 'local_courses'));
$returnurl = new moodle_url('/local/courses/courses.php');

// array of all valid fields for validation
$STD_FIELDS = array('coursecode','credits','Levelcode','duration_in_hours','duration_in_minutes');


$PRF_FIELDS = array();
//-------- if variable $iid equal to zero,it allows enter into the form -------

$mform1 = new local_courses\form\credits_form();
...
if ($formdata = $mform1->get_data()) {
    ...
    $progresslibfunctions = new local_courses\creditsbulkuploads\progresslibfunctions();
    $filecolumns = $progresslibfunctions->uu_validate_user_upload_columns($cir, $STD_FIELDS, $PRF_FIELDS, $returnurl);

    $hrms= new local_courses\creditsbulkuploads\cronfunctionality();
    ...
    echo html_writer::link(new moodle_url('/local/courses/creditsupload/sample.php?format=csv'),'Sample',array('id'=>'download_coursess'));
    echo html_writer::link(new moodle_url('/local/courses/creditsupload/help.php'),'Help manual' ,array('id'=>'download_coursess','target'=>'__blank'));
