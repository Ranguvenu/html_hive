...
if ($course) {
  ...
  // role selection
  // $select = new single_select(new moodle_url('/local/courses/courseenrol.php',
  //         array('id' => $course_id, 'enrolid' => $enrolid, 'sesskey' => sesskey())),
  //     'roleid',  $roles);
  // $select->label = get_string('role');
  // $content = $OUTPUT->render($select);
  ...
  $content.='<div class="box3 col-12 col-md-2 actions pull-left">
              <button type="submit" class="custom_btn btn remove btn-default" disabled="disabled" title="Remove Selected Users" name="submit_value" value="Remove Selected Users" id="user_unassign_all">
...
echo '<a class="btn-link btn-sm" title="Filter" href="javascript:void(0);" data-toggle="collapse" data-target="#local_coursesenrol-filter_collapse" aria-expanded="false" aria-controls="local_coursesenrol-filter_collapse">
