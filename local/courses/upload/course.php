...
class local_uploadcourse_course {
    ...
    static protected $validfields = array('fullname', 'course-code', 'idnumber', 'category', 'visible', 'startdate', 'enddate','open_points','coursetype','open_identifiedas','open_costcenterid','open_cost','open_requestcourseid','open_coursecreator','open_coursecompletiondays','open_departmentid','open_subdepartment','open_credits','open_facilitatorcredits','file',
        'summary', 'format', 'theme', 'lang', 'newsitems', 'showgrades', 'showreports', 'legacyfiles', 'maxbytes',
        'groupmode', 'groupmodeforce', 'enablecompletion', 'course_credits', 'duration_in_hours','duration_in_minutes');
    ...
    public function prepare() {
        ...
        // if(!empty($this->rawdata['duration'])){
        //     if(!preg_match("/^[0-9][0-9]*[0-9]:[0-5][0-9]$/", $this->rawdata['duration'])) {
        //         $this->error('durationshouldmatchformat', new lang_string('durationshouldmatchformat', 'local_courses', $this->rawdata['duration']));
        //         return false; 
        //     }else{
        //         sscanf($this->rawdata['duration'], "%d:%d", $hours, $minutes);
        //         if ($hours ==0 && $minutes==0) {
        //             $this->error('durationcannotbezero', new lang_string('durationcannotbezero', 'local_courses', $this->rawdata['duration']));
        //             return false; 
        //         }
        //     }
        // }else{
        //     $this->error('durationcannotbeempty', new lang_string('durationcannotbeempty', 'local_courses', $this->rawdata['duration']));
        //     return false;
        // }

        if (isset($this->rawdata['duration_in_minutes'])) {
            $duration = $this->rawdata['duration_in_minutes'];
    
            if (!is_numeric($duration) || (int)$duration < 0 || (int)$duration > 59) {
                $this->error('durationminutesshouldmatchformat', new lang_string('durationminutesshouldmatchformat', 'local_courses', $duration));
                return false;
            }
        } else {
            $this->error('durationcannotbeempty', new lang_string('durationminutescannotbeempty', 'local_courses'));
            return false;
        }

        if(!empty($this->rawdata['duration_in_hours'])){
             if(!is_numeric($this->rawdata['duration_in_hours']) || (int)$this->rawdata['duration_in_hours'] != $this->rawdata['duration_in_hours'] ||  (int)$this->rawdata['duration_in_hours'] <= 0 ){
                $this->error('durationhoursshouldmatchformat', new lang_string('durationhoursshouldmatchformat', 'local_courses', $this->rawdata['duration_in_hours']));
                return false; 
             }
         
        }else{
            $this->error('durationcannotbeempty', new lang_string('durationhourscannotbeempty', 'local_courses', $this->rawdata['duration_in_hours']));
            return false;
        }

        if(isset($this->rawdata['open_points'])){
            $this->rawdata['course_credits'] = $this->rawdata['open_points'];
        }

        if(isset($this->rawdata['open_credits'])){
            $this->rawdata['course_credits'] = $this->rawdata['open_credits'];
        }

        if (isset($this->rawdata['course_credits'])) {
            $credits = $this->rawdata['course_credits'];

            if (!is_numeric($credits) || (int)$credits != $credits || (int)$credits < 0) {
                $this->error('coursecreditscannotbenonnumericwithargs', new lang_string('coursecreditscannotbenonnumericwithargs', 'local_courses', $credits));
                return false;
            }
        } else {
            $this->error('coursecreditscannotbeempty', new lang_string('coursecreditscannotbeempty', 'local_courses'));
            return false;
        }

        if(isset($this->rawdata['open_facilitatorcredits'])){
            $this->rawdata['facilitator_credits'] = $this->rawdata['open_facilitatorcredits'];
        }
        // if(!empty($this->rawdata['facilitator_credits'] || $this->rawdata['facilitator_credits'] < 0)){
        //     if(!((int)$this->rawdata['facilitator_credits'] == $this->rawdata['facilitator_credits'] && (int)$this->rawdata['facilitator_credits'] > 0)){
        //         $this->error('facilitatorcreditscannotbenonnumericwithargs', new lang_string('facilitatorcreditscannotbenonnumericwithargs', 'local_courses', $this->rawdata['facilitator_credits']));
        //         return false; 
        //     }
        // }else{
        //     $this->error('facilitatorcreditscannotbeempty', new lang_string('facilitatorcreditscannotbeempty', 'local_courses', $this->rawdata['facilitator_credits']));
        //     return false;
        // }
        ...
        } else if(has_capability('local/costcenter:manage_ownorganization',$systemcontext)  && isset($this->rawdata['category'])){
            $categories = $categorylib->get_categories($USER->open_costcenterid);
            if(!in_array($this->rawdata['category'], $categories)){
                $this->error('canonlycreatecourseincategoryofsameorganisationwithargs', new lang_string('canonlycreatecourseincategoryofsameorganisationwithargs', 'local_courses', $this->rawdata['categoryname']));
                return false;
            }
        } else if(has_capability('local/costcenter:manage_owndepartments', $systemcontext) && isset($this->rawdata['category'])){
        ...
        //         return false;
        //     } else {
        //         $coursetypes = $DB->get_records_menu('local_course_types',array(),'id','shortname');
        //         if (!in_array($this->rawdata['coursetype'], $coursetypes)) {
        //             $this->error('cannotcreateorupdatecourse', new lang_string('cannotcreateorupdatecourse', 'local_courses', $this->rawdata['coursetype']));
        //             return false;
        //         }
        //     }
        ...
        if ($this->rawdata['coursetype']) {
             $params = array();
             $coursetype_sql = "SELECT id FROM {local_course_types} WHERE shortname = :shortname "; 
             $params['shortname'] = $this->rawdata['coursetype'];
             
             $coursetype = $DB->get_field_sql($coursetype_sql,$params);
    ...
    public function proceed() {
        ...
        $params = array();
        $coursetype_sql = "SELECT id FROM {local_course_types} WHERE shortname = :shortname "; 
        $params['shortname'] = $this->data['coursetype'];
        $coursetype = $DB->get_field_sql($coursetype_sql,$params);
        ...
        // $hours = $hours * 3600 ;
        // $minutes = $minutes * 60;
        $duration_in_hours = $this->data['duration_in_hours'];
        $duration_in_minutes = $this->data['duration_in_minutes'];
        ...
        if ($this->do === self::DO_DELETE) {
            ...
            $this->data['open_identifiedas'] = $coursetype;
