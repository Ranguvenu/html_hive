...
class user{
    ...
    public function inprogress_coursenames($userid) {
        ...
        $params = array();
        $couresparams = array();
        $sql = "SELECT DISTINCT(course.id),ue.userid, course.fullname, course.shortname as code, course.expirydate as expirydate,
                course.summary,ue.timecreated as enrolldate
                ...
                (CONCAT(',',course.open_identifiedas,',') LIKE CONCAT('%,',3,',%') OR CONCAT(',',course.open_identifiedas,',') LIKE CONCAT('%,',1,',%') )
                AND course.id > 1 " ;
    ...
    public function enrol_get_users_courses($userid, $count =false, $limityesno = false, $start = 0, $limit = 5) {
        ...
        $fromsql = " FROM {course} course
                    ...
                    AND course.open_identifiedas NOT IN (2,4)";
    ...
    public function user_modulewise_content($id,$start =0,$limit=5){
      ...
      foreach($usercourses['data'] as $course){
          ...
          $coursessarray['enrolldate'] = date('d-m-Y H:i:s',$course->enrolldate);
          ...
          if(!empty($course->completiondate)){
            $coursessarray['completiondate'] = date('d-m-Y H:i',$course->completiondate);
          }else{
