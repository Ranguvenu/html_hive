...
class userdashboard implements \renderable, \templatable {
    ...
    public function __construct($filter, $filter_text='', $filter_offset = 0, $filter_limit = 0, $coursestype = null){
        $this->inprogresscount = general_lib::inprogress_coursenames_count($filter_text, $coursestype);
        $this->completedcount = general_lib::completed_coursenames_count($filter_text, $coursestype);
        switch ($filter){
            case 'inprogress':
                $this->courseslist = general_lib::inprogress_coursenames($filter_text, $filter_offset, $filter_limit, $coursestype);
                $this->coursesViewCount = $this->inprogresscount;
                $this->subtab='elearning_inprogress';
                break;

            case 'completed' :                           
                $this->courseslist = general_lib::completed_coursenames($filter_text, $filter_offset, $filter_limit, $coursestype);
     ...
     * @return stdClass
     */
    public function export_for_template($output, $coursetype = null) {
        global $DB, $USER, $CFG, $PAGE;
        $PAGE->requires->js_call_amd('local/courses/courseAjaxform','load',array());
        ...
        if($coursetype == 'mooc'){
            $data->view_more_url = $CFG->wwwroot.'/local/courses/userdashboard.php?tab='.explode('_',$this->subtab)[1].'&type='.$coursetype;
        }else if($coursetype == 'ilt'){
            $data->view_more_url = $CFG->wwwroot.'/local/courses/userdashboard.php?tab='.explode('_',$this->subtab)[1].'&type='.$coursetype;
        }else{
            ...
            foreach ($this->courseslist as $inprogress_coursename) {
               ...
               // $onerow['course_url'] = $CFG->wwwroot.'/course/view.php?id='.$inprogress_coursename->id;
                ...
                if(class_exists($classname)){
                  ...
                  $currenttime = time();
                  $fullname =  $inprogress_coursename->fullname;
                   
                    if($inprogress_coursename->expirydate != 0 && $inprogress_coursename->expirydate > $currenttime){
                         $coursepageurl = new \moodle_url('/course/view.php', array('id'=>$inprogress_coursename->id));
                         $courselink = html_writer::link($coursepageurl, $inprogress_coursename->fullname, array('class'=>'coursesubtitle','title'=>$inprogress_coursename->fullname));
                        
                         $onerow['course_url'] = $courselink;
                       
                     } else if($inprogress_coursename->expirydate != 0 && $inprogress_coursename->expirydate < $currenttime){
                       
                        $courselink = html_writer::link('javascript:void(0)',$inprogress_coursename->fullname, array('title' => $inprogress_coursename->fullname, 'alt' => $inprogress_coursename->fullname,'class'=>'coursesubtitle','onclick' =>'(function(e){ require("block_userdashboard/userdashboard").courseexpiry({selector:\'courseselfenrol'.$inprogress_coursename->id.'\', courseid:'.$inprogress_coursename->id.',enroll:1,coursename:\''.$inprogress_coursename->fullname.'\' }) })(event)'));
                        $onerow['course_url'] = $courselink;

                     } else if($inprogress_coursename->expirydate == 0) {
        ...
        if($coursetype == 'mooc'){
            $data->menu_heading = get_string('mooc','block_userdashboard');
        }else if($coursetype == 'ilt'){
            $data->menu_heading = get_string('iltcourses','block_userdashboard');
        }else{
