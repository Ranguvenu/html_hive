...
$STD_FIELDS = array('skillcategory','skills');

$PRF_FIELDS = array();
//-------- if variable $iid equal to zero,it allows enter into the form -------
$PAGE->set_heading(get_string('uploadskill', 'local_skill'));
...
if ($formdata = $mform->get_data()) {
	...
	$hrms = new local_skill\upload\skillupload();
	$hrms->bulk_skill_upload($cir,$filecolumns, $formdata);

}
else{
	echo $OUTPUT->header();
	echo html_writer::link(new moodle_url('/local/skillrepository/index.php'),get_string('back','local_skill'),array('id'=>'download_skill'));
	echo html_writer::link(new moodle_url('/local/skill/sample.php?format=csv'),get_string('sample','local_skill'),array('id'=>'download_skill'));
	echo html_writer::link(new moodle_url('/local/skill/help.php'),get_string('help_manual','local_skill'),array('id'=>'download_skill','target'=>'__blank'));
