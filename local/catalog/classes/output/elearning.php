...
class elearning implements renderable{
    ...
    public function get_elearning_courselist_query( $perpage, $startlimit, $return_noofrecords=false, 
        $returnobjectlist=false, $mooc = 'all'){
        global $DB,$USER;
        ...
        $cfromsql = " from {course} c JOIN {enrol} en on en.courseid=c.id ";
        $usql = " UNION ";
        $tsql = " SELECT c.* ";

        $tfromsql = " from {course} c JOIN {enrol} en on en.courseid=c.id ";

        $tjoinsql = " JOIN {tag_instance} tgi ON tgi.itemid = en.courseid AND tgi.itemtype = 'courses' AND tgi.component = 'local_courses' JOIN {tag} t ON t.id = tgi.tagid ";
        ...
        $wheresql = " WHERE c.id > 1 and (en.enrol='self' and en.status=0) ";
        if($mooc == 'mooc'){
           $wheresql .= " AND CONCAT(',',c.open_identifiedas,',') LIKE '%,1,%' "; 
        }else if($mooc == 'elearning'){
            $wheresql .= " AND CONCAT(',',c.open_identifiedas,',') LIKE '%,3,%' ";
        }else{
            $wheresql .= " AND (CONCAT(',',c.open_identifiedas,',') LIKE '%,1,%' 
                                OR CONCAT(',',c.open_identifiedas,',') LIKE '%,3,%' ) ";
        }
       ...
       $wheresql .= " AND c.visible=1 AND (c.open_subdepartment = 0 $subdepartmentcond )";
        ...
        // $numofcourses=$DB->get_records_sql($csql.$cfromsql.$leftjoinsql.$wheresql.$cwrsql.$usql.$tsql.$tfromsql.$leftjoinsql.$wheresql.$twrsql);
    ...
    public function export_for_template($perpage,$startlimit, $mooc = 'all'){
    ...
    $courseslist_ar= $this->get_elearning_courselist_query( $perpage,$startlimit, true, true, $mooc);
    ...
    foreach ($courseslist as $course){         
        ...
        // $course->start_date = date("j M 'y", $courserecord->startdate);          
        ...
        $course->selfenrol=$this->get_enrollbutton($enroll, $course,$udemysync_license);
        ...
        // if($ratings_exist){
        //     $course->course_avgrating = display_averagerating($courserecord->id, 'course');
        // }else{
        //     $course->course_avgrating = null;
        // }
   ...
   public function get_enrollbutton($enroll, $courseinfo,$udemysync_license = false){
        global $DB,$CFG,$USER;
        ...
        if(!is_siteadmin()){
          if($courseinfo->approvalreqd==1){
            ...
            // $request = $DB->get_field('local_request_records','status',array('componentid' => $courseid,'compname' => $component,'createdbyid'=>$USER->id));
            if($request=='PENDING'){
