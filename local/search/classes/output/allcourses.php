    ...
    if($selectedfilter){
        ...
        foreach ($selectedfilter as $itemname) { 
         ...
         if($moduletype = $thisfilters['moduletype']){ 
            ...
            if($thisfilters['learningtype']){
                 if(!in_array(elearning, $standard_catalogtypes)){
                    $standard_catalogtypes[] = elearning; 
                 }
            }
        }else{
            ...
            //     }
            // }
        $custom_catalogtypes = $defaultPlugins;
        foreach($thisfilters AS $filtertype => $filter){
            if($filtertype == 'moduletype')
                continue;
            foreach($filterapplicable AS $plugin => $applicablefilters){
                if(!in_array($filtertype, $applicablefilters) && in_array($plugin, $custom_catalogtypes)){
                    unset($custom_catalogtypes[array_search($plugin, $custom_catalogtypes)]);
                }
            }
        }
        $standard_catalogtypes = array_intersect($standard_catalogtypes, $custom_catalogtypes);
       ...
       case learningplan :
        ...
        default:
            ...
            if(class_exists($classname)){
                $class = new $classname();
                $availabletypes[] = $type;
                $courseslist_ar = $class->get_elearning_courselist_query(1, searchlib::$page*1, true,true, $thisfilters);
...
public function main_toget_catalogtypes($perpage, $selectedfilter = array()){
    ...
    foreach($totalrecords_ineachtype as $index => $record){   
        ...
        if($index == 0){
            ...
            if($firstlvl_perpage){
                $firstresult_ar = $this->to_finding_specific_catalogtype($record['type'], $firstlvl_perpage,$firstlvl_startlimit, $thisfilters);
 ...
 private function to_finding_specific_catalogtype($recordtype, $level_perpage,$level_startlimit, $selectedfilter = array()){
     ...
     /*    foreach ($selectedfilter as $key=>$itemname) {  
            if($key == 'learningtype'){            
              
                if($itemname[0] == 4){
                    $recordtype = 'learningplan';
                }else if($itemname[0] == 2){               
                    $recordtype = 'classroom';
                }else{
                    $recordtype = 'elearning';
                }
            }
        }   */
