...
use local_percipiosync\output\percipiosyncview;

class local_percipiosync_external  extends external_api{
    /**
     * Returns description of method parameters.
     *
     * @return external_function_parameters.
     */
    public static function managepercipiolog_parameters() {
     ...
     */
    public static function managepercipiolog(
        $options,
        $dataoptions,
        $offset = 0,
        $limit = 0,
        ...
        require_login();
        $PAGE->set_url('/local/percipiosync/sync_history.php', array());
        $PAGE->set_context($contextid);
        // Parameter validation.
        $params = self::validate_parameters(
            self::managepercipiolog_parameters(),
            [
                'options' => $options,
        ...
        $stable = new \stdClass();
        $stable->thead = true;
        $totalsynchistory = (new percipiosyncview)::manage_percipiohistory_count($stable,$filtervalues);
        $totalsynchistorycount = $totalsynchistory['count'];

        $stable->thead = false;
        $stable->start = $offset;
        $stable->length = $limit;
        $data = (new percipiosyncview)::manage_percipiohistory_content($stable,$filtervalues);
     ...
     */
    public static function  managepercipiolog_returns() {
    ...
    public static function manageudeystatics(
        ...
        global $OUTPUT, $CFG, $DB,$USER,$PAGE;
        require_login();
        $PAGE->set_url('/local/percipiosync/sync_historystatistics.php', array());
        $PAGE->set_context($contextid);
        // Parameter validation.
        $params = self::validate_parameters(
        ...
        $stable = new \stdClass();
        $stable->thead = true;
        $totalsynchistorystatistics = (new percipiosyncview)::manage_synchistorystatistics_count($stable,$filtervalues);
        $totalsynchistorystatisticscount = $totalsynchistorystatistics['count'] ? $totalsynchistorystatistics['count'] : 0;

        $stable->thead = false;
        $stable->start = $offset;
        $stable->length = $limit;
        $data = (new percipiosyncview)::manage_synchistorystatistics_content($stable,$filtervalues);
        return [
